...
class AdminView extends Component<AdminViewProps> {
    ...
    componentDidMount = () => {
        ...
        socket.on('updated_config', ({ config, value }: { config: string, value: any }) => {
            this.setState((state: AdminViewState) => {
                if (state.configs === null) return {};
                const updatedConfigs = state.configs.map((el) => el.id === config ? { ...el, value: value } : el);
    ...
    updateConfig = (index: number, value: any) => {
        this.setState((state: AdminViewState) => {
            ...
            this.props.socket.emit('change_config', { config: updatedConfigs[index].id, value: value });
            return { configs: updatedConfigs }
        })
    }
    ...
    getControl = (config: IConfig<any>, index: number) => {
        switch (config.control) {
            case 'switch':
                return <Switch checked={config.value} onChange={(_, checked) => this.updateConfig(index, checked)} />;
        }
    }
