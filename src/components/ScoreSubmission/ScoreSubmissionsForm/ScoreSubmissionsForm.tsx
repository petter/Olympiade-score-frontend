...
const ScoreSubmissionsForm: FunctionComponent<
  ScoreSubmissionsFormProps
> = props => {
  ...
  const pointOptions = Array.from(
    Array((props.maxPoints as number) + 1).keys()
  ).map((_, i) => {
    return {
      value: i,
      label: i + ' poeng',
  ...
  return (
    <form className={styles.Form} onSubmit={props.onSubmit}>
      <label className={styles.Label}>Gruppe:</label>
      <Select
      ...
      <label className={styles.Label}>Poeng:</label>
      <Select
