...
const changeObjInArray = (
  array: GroupState[],
  id: string,
  fn: (el: GroupState) => GroupState
) => {
...
const reducer = (state = initialState, action: GroupActions) => {
  switch (action.type) {
    case 'group/setName':
      return changeObjInArray(state, action.payload.id, el =>
        changeField(el, 'name', action.payload.value)
      );
    ...
    case 'groups/setScore':
      return changeObjInArray(state, action.payload.id, el =>
        changeField(el, 'score', action.payload.value)
      );
    case 'groups/addScore':
      return changeObjInArray(state, action.payload.id, el =>
        changeField(el, 'score', el.score + action.payload.value)
      );
